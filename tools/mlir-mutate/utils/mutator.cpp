...
ReplaceValueMutation::ReplaceValueMutation(std::shared_ptr<FunctionMutator> mutator):
...
void ReplaceValueMutation::replaceOperand(mlir::Operation &op, size_t pos)
{
  ...
  mlir::Value newVal=mlir::Value(), oldVal = op.getOperand(pos);
  if (forceadd)
  {
  ...
  {
    if(newVal==mlir::Value()){
        newVal = mutator->addFunctionArgument(oldVal.getType());
    }
  }
  assert(newVal!=mlir::Value()&&"failed to find a value in replacement\n");
  op.setOperand(pos, newVal);
}

mlir::Value ReplaceValueMutation::insertOperation(mlir::Type type, mlir::OpBuilder& builder, int recursion_count){

    assert(false && "It should exist at least one result with the given type");
    return mlir::Value();
}
...
{
  llvm::errs() << "Value replacement: current value\n";
  mutator->funcItInTmp.getOperation().print(llvm::errs());
  endl;
}
...
bool FunctionMutator::canMutate(mlir::func::FuncOp &func)
{
...
FunctionMutator::FunctionMutator(mlir::func::FuncOp curFunc,
                                 mlir::IRMapping &irMap,llvm::DenseMap<mlir::Operation *,
                                 mlir::Operation *>& opMap)
    : curFunc(curFunc), irMap(irMap),opMap(opMap),
...
void FunctionMutator::resetCopy(std::shared_ptr<mlir::ModuleOp> tmpCopy)
{
  ...
  assert(irMap.contains(&funcIt.getBlock()) && "copy should be the same");
  auto block_it_in_tmp=irMap.lookup(&funcIt.getBlock())->getIterator();
...
mlir::Value FunctionMutator::addFunctionArgument(mlir::Type ty)
{
  ...
  assert(mlir::isa<mlir::func::FuncOp>(*op) && "operation should be a func::FuncOp");
  mlir::func::FuncOp funcInTmp = mlir::dyn_cast<mlir::func::FuncOp>(*op);
...
void FunctionMutator::moveToNextOperation()
{
...
void Mutator::recopy()
{
  irMap.clear();
  opMap.clear();
  mlir::Operation *copy = module.getOperation()->clone(irMap);
